-- Straight from examples/Introduction/Built-in.agda
...
-- All builtin operations are postulates => non-exportable for free
-- NATURAL, BOOL and LIST expect datatypes which are compiled into datatypes
-- along with convertion functions. 
-- INTEGER, FLOAT, CHAR, STRING, IO expect postulates

-- INTEGER, FLOAT, CHAR, STRING, IO cast respectively to Integer, Double, Char, String, IO
-- NATURAL, BOOL, LIST come with functions
-- mazNatToInteger, mazNatToInt, mazBoolToHBool, mazListToHList and
-- mazIntegerToNat, mazIntToNat, mazHBoolToBool, mazHListToList

-- NATURAL shouldn't be exported at all: mazIntegerToNat and mazIntToNat 
-- accept negative numbers and turn them into zero...

-- TODO: BOOL, LIST are not handled yet because it is essentially similar
-- TODO: to EXPORT_DATA
...
strToList = primStringToList

listToStr : List Char → String
listToStr = primStringFromList

len : (String → Int) → List Char → Int
len f lst = f (primStringFromList lst)

{-# EXPORT showChar showCharMy #-}
{-# EXPORT List List #-}
{-# EXPORT strToList strToList #-}
{-# EXPORT listToStr listToStr #-}
{-# EXPORT len len #-}
